        ...
        |((i, w), garblings)| {
            if i == finest_id {
                ...
                for part in 0..SUB_INPUT_GATES_PARTS {
                    ...
                    let mut file = mem_fs::MemFile::create(format!("babe_gates_{}.bin", part)).unwrap();
                    file.write_all(&bytes).unwrap();
                }

                bincode::serialize_into(
                    mem_fs::MemFile::create(format!("babe_wires.bin")).unwrap(),
                    &sub_wires,
                )
                    .unwrap();

                bincode::serialize_into(
                    mem_fs::MemFile::create(format!("babe_ciphertexts.bin")).unwrap(),
                    &ciphertexts,
                )
                    .unwrap();
